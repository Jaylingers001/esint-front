...
import BankAccountDao from "../dao/BankAccountDao";
...
const useBankAccount = () => {
    ...
    const loadBankDetails = () => {
        (async () => {
            if (loggedIn.isLoggedIn) {
                ...
                const dataBankLevel = await BankAccountDao().getAllBanks(GMO_BANKS.BANK_LEVEL);
                const dataBranchLevel = await BankAccountDao().getAllBanks(GMO_BANKS.BRANCH_LEVEL);
    ...
    const onSubmit = async (values: ApiMypageBankAccountToAdd) => {
        ...
        if (edit) {
            await router.push({pathname: '/mypage/bankAccount/edit'})
            return
        }
        if (bankValue) {
        ...
        const update = await BankAccountDao().updateBankAccount(values)
        if (update.status === 200) {
            await router.push({pathname: '/mypage/bankAccount/complete'})
        }
    }
